...
for attempt in $(seq 1 "$max_attempts"); do
    if "$script_dir/gpu-off-safe.sh" --quiet; then
...
notify-send -u normal -e "GPU Manager" "NVIDIA GPU stayed powered on after resume. It will be retried by low-power mode when idle." -i dialog-warning
