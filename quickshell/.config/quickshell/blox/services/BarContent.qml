...
QtObject {
    ...
    function panelActions() {
        if (openPanel === "updates") {
            ...
            return [action("Run updates", updateRun, {"icon": "󰇚"}), action("List updates", updateList, {"icon": ""})];
        }
        if (openPanel === "caffeine") return [action("Off", scriptRoot + "/status/caffeine.sh off", {"id": "off", "icon": "󰒲", "keepOpen": true}), action("30 mins", scriptRoot + "/status/caffeine.sh 30m", {"id": "30m", "icon": "󰔟", "keepOpen": true}), action("1 hr", scriptRoot + "/status/caffeine.sh 1h", {"id": "1h", "icon": "󰞌", "keepOpen": true}), action("Indefinite", scriptRoot + "/status/caffeine.sh indefinite", {"id": "indefinite", "icon": "󰒳", "keepOpen": true})];
        return [];
    }
}
